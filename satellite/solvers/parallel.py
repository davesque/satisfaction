...
from satellite.expr import And, CNF, Expr, Lit, Or
...
class DPLL(Solver):
    ...
    def check(self, expr: CNF | None = None) -> bool:
        ...
        while units := self.find_units(expr):
            expr = self.unit_propagate(units, expr)
    ...
    @staticmethod
    def find_units(and_expr: CNF) -> set[Lit]:
        units = {}
    ...
    @staticmethod
    def unit_propagate(units: set[Lit], and_expr: CNF) -> CNF:
        logger.debug("eliminating units: %s", units)

        and_args = []
        for or_expr in and_expr.args:
            if set(or_expr.args).intersection(units):
                # omit any clause containing a unit literal
                continue

            # filter any arg equal to a unit literal's negation
            or_args = (lit for lit in or_expr.args if ~lit not in units)
